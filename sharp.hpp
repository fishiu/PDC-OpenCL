...
const fil_t f[fil_size][fil_size] = {1, 1, 1, 1, -9, 1, 1, 1, 1};

void cpu_sharp(int* img, int* out, int in_size, int out_size) {
  for (int i = 0; i < out_size; ++i) {
    for (int j = 0; j < out_size; ++j) {
      out[i * out_size + j] = 0;
      for (int fi = 0; fi < fil_size; ++fi)
        for (int fj = 0; fj < fil_size; ++fj) out[i * out_size + j] += img[(i + fi) * in_size + j + fj] * f[fi][fj];
    }
  }
}
